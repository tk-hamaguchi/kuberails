...
module Kuberails::Adapters
  ...
  class KubeclientAdapter < Base
    class << self
      ...
      #
      def destroy_ingresses(label_selector)
        destroy_resources_with(kube_extensions_api_client, 'Ingress', label_selector)
      end
      ...
      # @return [Array] 削除したリソースのHashを配列にしたもの
      #
      def destroy_services(label_selector)
        destroy_resources_with(kube_client, 'Service', label_selector)
      end
      ...
      # @return [Array] 削除したリソースのHashを配列にしたもの
      #
      def destroy_deployments(label_selector)
        destroy_resources_with(kube_extensions_api_client, 'Deployment', label_selector)
      end
      ...
      # @return [Array] 削除したリソースのHashを配列にしたもの
      #
      def destroy_replica_sets(label_selector)
        destroy_resources_with(kube_extensions_api_client, 'ReplicaSet', label_selector)
      end
      ...
      # @return [Array] 削除したリソースのHashを配列にしたもの
      #
      def destroy_pods(label_selector)
        destroy_resources_with(kube_client, 'Pod', label_selector)
      end
      ...
      def destroy_resources_with(client, resource_type_obj, label_selector)
        results        = []
        resource_type  = sanitize_resource_type(resource_type_obj)
        resource_types = resource_type.downcase.pluralize
